AliasAddr= =0x42000000+ (A-0x40000000)*8*4 +n*4		//外设位带别名区地址
AliasAddr= =0x22000000+ (A-0x20000000)*8*4 +n*4		//SRAM 位带别名区地址
...
{			
	...
	RCC_AHB1PeriphClockCmd( RCC_AHB1Periph_GPIOH, ENABLE); 	// 开启GPIO的时钟
								   
	GPIO_InitStructure.GPIO_Pin   = GPIO_Pin_10;		// 选择要控制的IO口				
	GPIO_InitStructure.GPIO_Mode  = GPIO_Mode_OUT;		// 设置IO口输出   	
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;		// 设置IO输出为推挽模式	
	GPIO_InitStructure.GPIO_PuPd  = GPIO_PuPd_UP;		// IO 内部上拉
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz; 	// IO 输出速率为50M

	GPIO_Init(GPIOH, &GPIO_InitStructure);		// 初始化IO口	  
	GPIO_SetBits(GPIOH, GPIO_Pin_10);			// IO 默认输出高电平		
}
